...
type InputOption struct {
	...
	FilterOr          bool   `long:"filter-or" description:"Bool, combine multiple --filter rules with OR instead of AND"`
...
type OutputOption struct {
	...
	Outputf         string `short:"o" long:"output" default:"default" description:"String, command-line output format; scan default: full, -F default: color"`
	FileOutputf     string `short:"O" long:"file-output" default:"default" description:"String, file output format; inferred from filename when omitted"` // 输出格式
	...
	Compress        bool   `short:"C" long:"compress" description:"Bool, disable compressed output"`
	...
	NoGuess         bool   `long:"no-guess" description:"Bool, omit guessed frameworks when formatting results"`
}
...
type SmartOption struct {
	...
	NoScan    bool   `short:"n" long:"no" description:"Bool, only run smart discovery and skip the following default port scan"`
...
type AdvanceOption struct {
	...
	ExploitName string   `short:"E" long:"exploit-name" description:"String, specify neutron template name, id, or tag"` // 指定漏扫poc名字
	ExploitFile string   `long:"ef" description:"File, load specified neutron templates"`
	FingerFile  []string `long:"ff" description:"File, load specified finger file; repeatable"` // 指定漏扫文件
	Payloads    []string `long:"payload" description:"String, specify neutron payload key=value; repeatable"`
	AttackType  string   `long:"attack-type" choice:"sniper" choice:"clusterbomb" choice:"pitchfork" description:"attack type"`
	Extract     []string `long:"extract" description:"String, extractor preset or custom regexp; repeatable"`
	//SuffixStr   string   `long:"suffix" description:"String, url path"`
	Opsec         bool     `long:"opsec" description:"Bool, opsec mode"`
	Filters       []string `long:"filter" description:"String, retain matching -F/-j results; repeatable"`
	OutputFilters []string `long:"output-filter" description:"String, suppress matching scan results from output; repeatable"`
	ScanFilters   []string `long:"scan-filter" description:"String, stop deep scanning and suppress matching results; repeatable"`
}
...
type MiscOption struct {
	...
	Printer     string   `short:"P" long:"print" choice:"port" choice:"workflow" choice:"neutron" choice:"nuclei" choice:"extract" description:"String, show preset config; nuclei is a legacy alias of neutron"` // 输出特定的预设
...
type ConfigOption struct {
	...
	Verbose    []bool `short:"v" long:"verbose" description:"Bool, enable active finger scan; repeat for level 2"`                     // version level1
